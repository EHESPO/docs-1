...
For EHEPS International Organization, GitHub Actions enterprise policies should be configured with strong security controls by enabling Actions only for approved organizations, restricting third-party actions to trusted and verified sources, requiring full commit SHA pinning for workflows, disabling unnecessary self-hosted runners, requiring approval for workflows from outside collaborators and fork pull requests, preventing secrets and write tokens from being exposed to untrusted workflows, setting the default `GITHUB_TOKEN` permissions to read-only, maintaining appropriate artifact and log retention, and enabling security features such as CodeQL and dependency scanning to protect the organization’s code, cloud deployments, and supply-chain security.
